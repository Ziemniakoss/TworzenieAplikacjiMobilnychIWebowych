...
public class CRegister {
	...
	@PostMapping
	public String registerUser(@Valid @ModelAttribute("user") RegisterRequest registerRequest, Errors errors, Model model) {
		...
		try {
			...
			HttpEntity<RegisterRequest> e = new HttpEntity<>(registerRequest, headers);

			response = rt.postForObject(url, e, RegisterResponse.class);
			...
			if (e.getStatusCode() == HttpStatus.BAD_REQUEST) {
				...
				try {
					response = mapper.readValue(e.getResponseBodyAsString(), RegisterResponse.class);
					model.addAttribute("errorlist", response.getErrors());
